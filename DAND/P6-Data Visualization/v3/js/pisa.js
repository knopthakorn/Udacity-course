...
var barPlotTitle = ["Arrives Late",
                    ...
                    "Gives Opp. to Express Opinions",
                    "Homework Not Back in Time"];


var conuntryList = ["Country",
                    "Albania",
                    "Argentina",
                    "Australia",
                    "Austria",
...
var findingTopics = [
   ...
   "Teacher Gives Opportunity to Express Opinions.",
   "Homework Once a Week Not Back in Time.",
   "Try it by your self."
   ];


// summary of finding topics
var findingSummary = [
  ...
  "The Classroom Management - Students Are Calm/Teacher Arrives late.",
  "Teacher-Directed Instruction, Sets Clear Goals to student for their stduying.",
...
var customFill = d3.scale.category10().domain(["Average", "Math", "Science", "Reading"]);

// data of participent by student count
var student = [{label :"America" , total : 76154.0},
                 {label :"Asia" , total : 66350.0},
                 {label :"Europe" , total: 238765.0},
                 {label :"Middle East & Africa", total: 38966.0},
                 {label :"Oceania" , total: 18772.0},
                 {label :"South America" , total: 46483.0}];

// data of participent by school count
var school = [{label :"America" , total: 2937.0},
                 {label :"Asia" , total : 2022.0},
                 {label :"Europe" , total: 8750.0},
                 {label :"Middle East & Africa", total: 1173.0},
                 {label :"Oceania" , total: 952.0},
                 {label :"South America" , total: 1832.0}];
...
// topic index for display visualization
var topicIndex = 9;
...
function clearAllSVG() {
  ...
  d3.select("#introview").select("svg").remove();
...
function plotData(d)
{
  ...
  {
    clearAllSVG();
    d3.select("#introview").classed("hidden", false);
  ...
  {
    clearAllSVG();
    d3.select("#introview").classed("hidden", true);
    d3.select("#analyze").classed("hidden", true);
...
// create donut plot
function makeDonutChart (tag, fact, w, h, r, sx, sy, tx, ty, info) {
  ...
  var path=svg.selectAll('path')
              ...
              .append('path')
              .attr('d', 'arc')
              .attr('fill', function(d, i){return colorFill(d.data.label);});

  path.transition()
      .duration(500)
      .attrTween('d', function(d) { var interpolate = d3.interpolate({startAngle: 0, endAngle: 0}, d);
...
function introductionView()
{
  ...
  // plot student
  makeDonutChart ("#plot1", student, 300, 300, radius, 150, 100 );
  makeDonutTitle ("#introview #info2", "Around 510,000 students globally", 200, 20);
  // plot school
  makeDonutChart ("#plot2", school, 500, 400, radius, 200, 100, -110, 100);
    makeDonutTitle ("#introview #info3", "Around 20,000 school globally", 50, 20 );
  // plot world map
...
function tryByYourSelf()
{
  ...
  d3.csv('data/pisa2012_v2.csv', function(error, data) {
    ...
    var overalTeacher = [Math.round(d3.sum(data.map(function(d){ return d.ST84Q03}))/64),
                         ...
                         Math.round(d3.sum(data.map(function(d){ return d.ST83Q04}))/64),
    ...
    //countries.text(function (d) { return d.Country; }).attr("value", function (d) { return d.Country; });
    // plot student
    makeDonutChart2 ("#customize #cplot1", acheivement, 200, 200,-50, 100);
    makeDonutChart2 ("#customize #cplot2", cntParty, 200, 200,-50, 100);
    ...
    d3.select('select').on("change", function() {
      ...
      var bplotData = [Math.round((selectItem.ST84Q03 *100.01)/100),
                     ...
                     Math.round((selectItem.ST83Q04 *100.01)/100),
      ...
      makeDonutChart2 ("#customize #cplot1", cntTeacher, 200, 200,-50, 100);
      makeDonutChart2 ("#customize #cplot2", cntParty, 200, 200,-50, 100);
...
function makeBarPlot(barData, textInfo) {
  ...
  //xAxisLimit = d3.scale.linear().domain([30, 100]).range([1, 600]);
    ...
    var yscale = d3.scale.linear().domain([0,barPlotTitle.length]).range([0,300]);
    ...
    var x_xis = svg.append('g')
                   .attr("transform", "translate(100, 295)")
                   .attr('id','xaxis')
                   .call(xAxis);
    ...
    var transitext = d3.select('#bars')
              ...
              .enter()
              .append('text')
              .attr({'x':function(d) {return xscale(d)-150; },'y':function(d,i){ return yscale(i) +5; }})
...
// create donut plot
function makeDonutChart2 (tag, fact, w, h, tx, ty) {
var dataset = [
