...
def autoTest():
    for pidx, (pname, _) in enumerate(PROBLEMS):
        p_choices = str(pidx)
        for sidx, (sname, _, heuristic) in enumerate(SEARCHES):
            s_choices = str(sidx)

            print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            print("\n  ++++ evaluation result  ", "{!s}. {}".format(pidx+1, pname), "    {!s}. {} {}".format(sidx+1, sname, heuristic))
            main(p_choices, s_choices)
